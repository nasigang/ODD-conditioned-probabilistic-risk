...
def _infer_flow_arch_and_hparams_from_sd_by_shape(flow_sd: Dict[str, torch.Tensor], *, x_dim: int, c_dim: int, cond_dim: int) -> Tuple[str, int, int, int, int, int]:
    ...
    for k, v in flow_sd.items():
        if k.endswith("weight") and getattr(v, "ndim", 0) == 2:
            ...
            mats.append((k, o, i))
    if not mats:
        raise RuntimeError("Flow checkpoint has no 2D weights; cannot infer architecture.")

    # out_dim = 3K+1
    out_cands = [(o, k) for (k, o, i) in mats if o >= 10 and ((o - 1) % 3 == 0)]
    if not out_cands:
        raise RuntimeError("Could not infer num_bins: no weight with out_dim=3K+1 found.")
...
def load_models_for_run(run_dir: Path, state, device: torch.device):
    ...
    - Expert Flow: ConditionalSpline1DFlow (concat or FiLM)
    """
    ...
    if flow_ckpt.exists():
        ...
        flow_x_cols = state.schema.flow_x_cols_in_order()
        flow_c_cols = state.schema.flow_c_cols_in_order()
        x_dim = int(len(flow_x_cols))
...
def infer_flow_cdf_probs(
    ...
    *,
    ...
    """Compute p_event = P(Y <= y_thr | cond) using Flow CDF.

    Supports:
      - concat mode: cond is a single tensor (B, C)
      - FiLM mode:   cond is a tuple (x_part, c_part)
    """
    n = x_ctx_raw.shape[0]
    out = np.empty((n,), dtype=np.float32)
    flow.eval()
    ...
    for i0 in range(0, n, batch):
        ...
        # fingerprint suppression: set dropped dims to train-mean
        if drop_idx is not None and drop_idx.numel() > 0:
            xe_raw = xe_raw.clone()
        ...
        cdf = flow.cdf(yb, cond).reshape(-1)
    ...
    for i0 in range(0, n, batch):
        ...
        if film_mode:
            if flow_x_idx is None or flow_c_idx is None:
                raise RuntimeError("Flow is in FiLM mode but flow_x_idx/flow_c_idx were not provided.")
            xb_cond = (
                xb.index_select(1, flow_x_idx),
                xb.index_select(1, flow_c_idx) if int(flow_c_idx.numel()) > 0 else xb[:, :0],
            )
        else:
            xb_cond = xb
...
def main():
    ...
    for run in args.run:
        ...
        # Flow split indices (for FiLM mode). These are indices into ds.tensors.x_expert_raw.
        # If the Flow is concat-mode, these are ignored.
        ...
        try:
            ...
            print(f"[warn] Could not build Flow split indices: {e}. Falling back to concat.")
            flow_x_idx_t, flow_c_idx_t = None, None
